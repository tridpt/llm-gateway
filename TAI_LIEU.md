...
## 4h. Endpoint Anthropic `/v1/messages` (format translation 2 chiều)

Ngoài API kiểu OpenAI, gateway còn nhận request theo **định dạng Anthropic Messages API**. Client kiểu Claude SDK / Claude Code có thể trỏ thẳng vào gateway.

Luồng: request Anthropic → dịch sang định dạng nội bộ (OpenAI-style: gộp `system` thành message, gộp content block thành chuỗi) → chạy qua **toàn bộ pipeline** (routing, fallback, cache, budget, token saver) → dịch kết quả ngược lại định dạng Anthropic (cả response thường lẫn SSE stream).

- Non-stream: trả `{id:"msg_...", type:"message", content:[{type:"text"}], stop_reason, usage}`.
- Stream: phát các event Anthropic (`message_start`, `content_block_delta`, `message_delta`, `message_stop`).

**Vì sao tách helper `openStreamAcrossTargets`?**
Cả endpoint OpenAI và Anthropic đều cần "mở stream qua chuỗi target có fallback + timeout + circuit breaker". Gom vào một helper dùng chung → không lặp code, sửa một chỗ áp dụng cả hai. Đây là nguyên tắc DRY và là điểm hay để nói khi phỏng vấn về refactor.

**Gateway là "adapter hai chiều":**
Nhận nhiều định dạng đầu vào (OpenAI, Anthropic), chuẩn hóa về một format nội bộ, gọi nhiều provider có format khác nhau, rồi trả về đúng format client mong đợi. Đây chính là giá trị "universal API" của OpenRouter/9router.
...
- `src/routes/` — chat (lõi), embeddings, models, anthropic, admin
...
19. Gateway nhận được cả định dạng OpenAI lẫn Anthropic — giải thích cơ chế "adapter 2 chiều".
20. Vì sao streaming fallback chỉ làm được trước chunk đầu tiên?
