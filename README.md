...
- [Team Chat](docs/TEAM_CHAT.md) — self-hosted ChatGPT UI, team management, sync, encryption at rest
...
- Team chat: <http://localhost:8080/chat>
...
that runs entirely on top of the gateway. Provider API keys stay on the server
(shared), while each member signs in with their own credentials, so usage and
cost count against that person's daily budget. It streams over SSE, syncs
conversations across devices, includes a runtime team-admin panel, and supports
AES-256-GCM encryption at rest for the JSON stores.

See **[docs/TEAM_CHAT.md](docs/TEAM_CHAT.md)** for the full guide — login,
managing members, conversation sync, and encryption at rest.
